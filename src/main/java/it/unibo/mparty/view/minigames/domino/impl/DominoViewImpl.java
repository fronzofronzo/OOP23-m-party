    ...
    public void setPlayerTiles(boolean isPlayer1, Set<Tile> playerTiles) {
        if (isPlayer1) {
            ...
            for (Tile tile : playerTiles) {
                this.player1Tiles.getChildren().add(generateDomino(tile.getSideA().getValue(), tile.getSideB().getValue()));
            }
        } else {
            this.player2Tiles.getChildren().clear();
            for (Tile tile : playerTiles) {
                this.player2Tiles.getChildren().add(generateDomino(tile.getSideA().getValue(), tile.getSideB().getValue()));
            }
        }
    }
